...
# Generate disorder sequences
Y = [generate_correlated_disorder(Num_sites=301, Mean=0, correlation_length=3, Std_deviation=0.5, N=5)for _ in range(num_sequences)]
print(len(Y))
...
def compute_cond_fine(args):
    ...
    Vdis = disorder_pot(Y, num_sites=301)
    
    sys = BdG_conds_1D(Vw, mu_w, Vdis).finalized()
